...
int CHudDeathNotice :: Init( void )
{
	...
	m_pCvarKillMessage = gEngfuncs.pfnRegisterVariable( "ebash3d_killmessage", "0", FCVAR_ARCHIVE );
	
	// Инициализируем генератор случайных чисел для kill message (один раз при инициализации)
...
int CHudDeathNotice :: MsgFunc_DeathMsg( const char *pszName, int iSize, void *pbuf )
{
	...
	// ИСПРАВЛЕНИЕ ESP: Инвалидируем кэш позиции для убитого игрока
	// Это предотвращает отображение ESP для мертвых игроков на серверах
	// Кэш будет обновлен только когда игрок возродится и его позиция обновится
	extern bool g_PlayerPositionCacheValid[];
	extern float g_PlayerPositionCache[];
	if (victim > 0 && victim <= MAX_PLAYERS)
	{
		g_PlayerPositionCacheValid[victim] = false;
		// Обнуляем позицию в кэше для полной очистки
		g_PlayerPositionCache[victim * 3 + 0] = 0.0f;
		g_PlayerPositionCache[victim * 3 + 1] = 0.0f;
		g_PlayerPositionCache[victim * 3 + 2] = 0.0f;
	}
	...
	// Kill Message - отправляем сообщение в чат, если локальный игрок убил кого-то
	if (m_pCvarKillMessage && m_pCvarKillMessage->value != 0.0f)
	{
		// Проверяем, является ли локальный игрок киллером
		// killer - это индекс игрока в g_PlayerInfoList (1-MAX_PLAYERS)
		if (killer > 0 && killer <= MAX_PLAYERS && 
		    victim > 0 && victim <= MAX_PLAYERS && victim != killer &&
		    !rgDeathNoticeList[i].bSuicide && !rgDeathNoticeList[i].bTeamKill && 
		    !rgDeathNoticeList[i].bNonPlayerKill)
		{
			// Проверяем, что локальный игрок является киллером
			// g_PlayerInfoList[killer].thisplayer - флаг, указывающий, является ли этот игрок локальным
			if (g_PlayerInfoList[killer].thisplayer)
			{
				// Проверяем, что имя жертвы не пустое
				if (rgDeathNoticeList[i].szVictim && strlen(rgDeathNoticeList[i].szVictim) > 0)
				{
					// Передаем имя жертвы в функцию отправки сообщения
					SendKillMessage(rgDeathNoticeList[i].szVictim);
				}
			}
		}
	}
...
void CHudDeathNotice::SendKillMessage(const char *victimName)
{
	// Проверяем, что имя жертвы валидно
	if (!victimName || strlen(victimName) == 0)
		return;
	
	// Список сообщений для случайного выбора
	const char *killMessages[] = {
		"i f u c k e d your dead family",
		"stop s u c k",
		"buy eBash3D",
		"you are a weak suicide please",
		"your mother appreciated my p e n i s"
	};
	int messageCount = sizeof(killMessages) / sizeof(killMessages[0]);
	
	// Выбираем случайное сообщение
	int randomMessageIndex = rand() % messageCount;
	const char *selectedMessage = killMessages[randomMessageIndex];
	
	// Генерируем рандомные буквы и цифры в конце сообщения для обхода антифлуда
	// Длина рандомной строки: 4-10 символов
	int randomLength = 4 + (rand() % 7);  // 4-10 символов
	char randomSuffix[16] = {0};
	
	// Генерируем рандомные символы (a-z, A-Z, 0-9) для более разнообразного обхода антифлуда
	const char charset[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
	int charsetSize = sizeof(charset) - 1;
	
	for (int i = 0; i < randomLength; i++)
	{
		randomSuffix[i] = charset[rand() % charsetSize];
	}
	randomSuffix[randomLength] = '\0';
	
	// Формируем сообщение: имя жертвы + случайное сообщение + рандомные символы
	// Формат: "[имя жертвы] [случайное сообщение] [рандомные символы]"
	char message[512];
	snprintf(message, sizeof(message), "say \"%s %s %s\"", victimName, selectedMessage, randomSuffix);
	
	// Отправляем команду в чат
	gEngfuncs.pfnClientCmd(message);
}
