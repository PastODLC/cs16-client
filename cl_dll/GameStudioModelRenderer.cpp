...
// Chams CVARs
static cvar_t *ebash3d_chams = nullptr;
static cvar_t *ebash3d_chams_ct_r = nullptr;
static cvar_t *ebash3d_chams_ct_g = nullptr;
static cvar_t *ebash3d_chams_ct_b = nullptr;
static cvar_t *ebash3d_chams_t_r = nullptr;
static cvar_t *ebash3d_chams_t_g = nullptr;
static cvar_t *ebash3d_chams_t_b = nullptr;
static cvar_t *ebash3d_chams_team_r = nullptr;
static cvar_t *ebash3d_chams_team_g = nullptr;
static cvar_t *ebash3d_chams_team_b = nullptr;
static cvar_t *ebash3d_chams_alpha = nullptr;
static cvar_t *ebash3d_chams_distance = nullptr;

// Функция для регистрации CVARs чамсов
static void RegisterChamsCvars()
{
	if (!ebash3d_chams)
	{
		ebash3d_chams = gEngfuncs.pfnRegisterVariable("ebash3d_chams", "0", FCVAR_ARCHIVE);
		ebash3d_chams_ct_r = gEngfuncs.pfnRegisterVariable("ebash3d_chams_ct_r", "0", FCVAR_ARCHIVE);
		ebash3d_chams_ct_g = gEngfuncs.pfnRegisterVariable("ebash3d_chams_ct_g", "0", FCVAR_ARCHIVE);
		ebash3d_chams_ct_b = gEngfuncs.pfnRegisterVariable("ebash3d_chams_ct_b", "255", FCVAR_ARCHIVE);
		ebash3d_chams_t_r = gEngfuncs.pfnRegisterVariable("ebash3d_chams_t_r", "255", FCVAR_ARCHIVE);
		ebash3d_chams_t_g = gEngfuncs.pfnRegisterVariable("ebash3d_chams_t_g", "0", FCVAR_ARCHIVE);
		ebash3d_chams_t_b = gEngfuncs.pfnRegisterVariable("ebash3d_chams_t_b", "0", FCVAR_ARCHIVE);
		ebash3d_chams_team_r = gEngfuncs.pfnRegisterVariable("ebash3d_chams_team_r", "0", FCVAR_ARCHIVE);
		ebash3d_chams_team_g = gEngfuncs.pfnRegisterVariable("ebash3d_chams_team_g", "255", FCVAR_ARCHIVE);
		ebash3d_chams_team_b = gEngfuncs.pfnRegisterVariable("ebash3d_chams_team_b", "0", FCVAR_ARCHIVE);
		ebash3d_chams_alpha = gEngfuncs.pfnRegisterVariable("ebash3d_chams_alpha", "255", FCVAR_ARCHIVE);
		ebash3d_chams_distance = gEngfuncs.pfnRegisterVariable("ebash3d_chams_distance", "10000", FCVAR_ARCHIVE);
	}
}

// Функция проверки видимости игрока
static bool IsPlayerVisible(cl_entity_t* target)
{
	if (!target)
		return false;
	...
	targetPos[2] += 28.0f; // Высота головы
	...
	if (traceDistance > 8192.0f)
		traceDistance = 8192.0f;
	...
	// Если trace прошел достаточно далеко, считаем видимым
	if (traceData.fraction >= 0.95f)
	{
		return true;
	}
...
int CGameStudioModelRenderer::_StudioDrawPlayer(int flags, entity_state_t *pplayer)
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		// Применяем чамсы к игрокам в заданном радиусе (видимые и невидимые)
		bool chamsApplied = false;
		if (ebash3d_chams && ebash3d_chams->value != 0.0f)
		{
			cl_entity_t* localPlayer = gEngfuncs.GetLocalPlayer();
			if (localPlayer && m_pCurrentEntity->index != localPlayer->index)
			{
				// Проверяем расстояние и здоровье игрока
				// Получаем позицию локального игрока (глаза)
				float localOrigin[3];
				VectorCopy(localPlayer->origin, localOrigin);
				...
				// Получаем позицию цели
				float targetPos[3];
				VectorCopy(m_pCurrentEntity->origin, targetPos);
				targetPos[2] += 17.0f; // Высота глаз
				...
				if (distance <= ebash3d_chams_distance->value)
				{
					// Сохраняем оригинальные параметры рендеринга
					int oldRenderMode = m_pCurrentEntity->curstate.rendermode;
					int oldRenderFx = m_pCurrentEntity->curstate.renderfx;
					int oldRenderAmt = m_pCurrentEntity->curstate.renderamt;
					color24 oldRenderColor = m_pCurrentEntity->curstate.rendercolor;

					// Определяем команду игрока
					extern extra_player_info_t g_PlayerExtraInfo[MAX_PLAYERS + 1];
					int targetTeam = g_PlayerExtraInfo[m_pCurrentEntity->index].teamnumber;
					int localTeam = g_PlayerExtraInfo[localPlayer->index].teamnumber;

					// Выбираем цвет в зависимости от команды
					int r, g, b;
					if (targetTeam == localTeam)
					{
						// Свои - зеленый (или настраиваемый)
						r = (int)(ebash3d_chams_team_r ? ebash3d_chams_team_r->value : 0);
						g = (int)(ebash3d_chams_team_g ? ebash3d_chams_team_g->value : 255);
						b = (int)(ebash3d_chams_team_b ? ebash3d_chams_team_b->value : 0);
					}
					else if (targetTeam == 2) // CT
					{
						// CT - синий
						r = (int)(ebash3d_chams_ct_r ? ebash3d_chams_ct_r->value : 0);
						g = (int)(ebash3d_chams_ct_g ? ebash3d_chams_ct_g->value : 0);
						b = (int)(ebash3d_chams_ct_b ? ebash3d_chams_ct_b->value : 255);
					}
					else // Terrorist
					{
						// T - красный
						r = (int)(ebash3d_chams_t_r ? ebash3d_chams_t_r->value : 255);
						g = (int)(ebash3d_chams_t_g ? ebash3d_chams_t_g->value : 0);
						b = (int)(ebash3d_chams_t_b ? ebash3d_chams_t_b->value : 0);
					}

					int alpha = (int)(ebash3d_chams_alpha ? ebash3d_chams_alpha->value : 255);

					// Применяем цветной эффект
					m_pCurrentEntity->curstate.rendermode = kRenderTransColor;
					m_pCurrentEntity->curstate.renderfx = kRenderFxGlowShell;
					m_pCurrentEntity->curstate.rendercolor.r = (byte)r;
					m_pCurrentEntity->curstate.rendercolor.g = (byte)g;
					m_pCurrentEntity->curstate.rendercolor.b = (byte)b;
					m_pCurrentEntity->curstate.renderamt = (byte)alpha;

					chamsApplied = true;

					// Рендерим модель с чамсами
					StudioRenderModel(dir);

					// Восстанавливаем оригинальные параметры
					m_pCurrentEntity->curstate.rendermode = oldRenderMode;
					m_pCurrentEntity->curstate.renderfx = oldRenderFx;
					m_pCurrentEntity->curstate.renderamt = oldRenderAmt;
					m_pCurrentEntity->curstate.rendercolor = oldRenderColor;
				}
			}
		}

		// Если чамсы не применены, рендерим нормально
		if (!chamsApplied)
		{
			StudioRenderModel(dir);
		}
...
void CGameStudioModelRenderer::StudioRenderFinal_Hardware(void)
{
	int i;
	int rendermode;
