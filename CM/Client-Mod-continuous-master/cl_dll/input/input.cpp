...
void DLLEXPORT CL_CreateMove( float frametime, struct usercmd_s *cmd, int active )
{
	...
	if( active )
	{
		...
		// Инициализируем cmd->viewangles текущими углами
		VectorCopy(viewangles, cmd->viewangles);
		
		// Сохраняем визуальные углы для silent aim (камера не должна двигаться)
		vec3_t savedVisualAngles;
		VectorCopy(viewangles, savedVisualAngles);
		
		// Применяем аимбот/системы ДО обработки движения
		if( g_iAlive )
		{
			bool aimbotEnabled = gHUD.m_Aimbot.IsEnabled();
			bool rcsEnabled = gHUD.m_Aimbot.IsRCSEnabled();
			bool noRecoilEnabled = gHUD.m_Aimbot.IsNoRecoil();
			bool silentAimEnabled = gHUD.m_Aimbot.IsSilentAim();
			
			// Обновляем состояние стрельбы для аимбота (предварительное)
			extern bool g_AimbotIsShooting;
			g_AimbotIsShooting = (CL_ButtonBits( 0 ) & IN_ATTACK) != 0;
			
			// ВСЕГДА вызываем Think() если silent aim включен (для стабильной работы)
			// Также вызываем для аимбота, RCS, No Recoil
			bool shouldCallThink = silentAimEnabled || aimbotEnabled || rcsEnabled || noRecoilEnabled;
			
			if (shouldCallThink)
			{
				// Проверяем условие для аимбота: только при стрельбе или всегда
				bool shouldAim = !aimbotEnabled || !gHUD.m_Aimbot.IsOnlyWhenShooting() || g_AimbotIsShooting;
				
				// Вызываем Think() если нужно
				// Для silent aim ВСЕГДА вызываем (для стабильного обновления углов)
				bool callThink = silentAimEnabled || shouldAim || (rcsEnabled && g_AimbotIsShooting) || noRecoilEnabled;
				
				if (callThink)
				{
					// Вызываем Think аимбота для наведения (также применяет No Recoil, RCS, Silent Aim)
					gHUD.m_Aimbot.Think();
					
					// Если включен Silent Aim, ВСЕГДА применяем углы к cmd->viewangles
					if (silentAimEnabled)
					{
						// Получаем целевые углы из аимбота (вычислены в Think())
						vec3_t aimAngles;
						gHUD.m_Aimbot.GetAimAngles(aimAngles);
						
						// Нормализуем углы для правильной передачи на сервер
						...
						// Применяем углы напрямую к cmd->viewangles (silent aim)
						// Это отправляется на сервер для регистрации попаданий
						VectorCopy(aimAngles, cmd->viewangles);
					}
					else
					{
						// Обычный режим - получаем обновленные углы после наведения
						gEngfuncs.GetViewAngles((float *)viewangles);
						VectorCopy(viewangles, cmd->viewangles);
					}
				}
				...
				{
					// Условие не выполнено - копируем текущие углы
					VectorCopy(viewangles, cmd->viewangles);
				}
			}
			else
			{
				// Все системы выключены - копируем текущие углы
				VectorCopy(viewangles, cmd->viewangles);
			}
		}
		
		// ВАЖНО: Для silent aim визуальные углы устанавливаем ДО IN_Move
		// Они будут восстановлены ПОСЛЕ IN_Move
		// Для обычного режима обновляем визуальные углы
		if (!(g_iAlive && gHUD.m_Aimbot.IsSilentAim()))
		{
			// Обычный режим - обновляем визуальные углы
			gEngfuncs.SetViewAngles((float *)viewangles);
		}
		// Для silent aim визуальные углы будут восстановлены после IN_Move

		// Применяем спидхак если включен
		float speedMultiplier = 1.0f;
		bool speedHackEnabled = gHUD.m_Aimbot.IsSpeedHack();
		if (speedHackEnabled)
		{
			speedMultiplier = gHUD.m_Aimbot.GetSpeedMultiplier();
		}

		if( in_strafe.state & 1 )
		{
			cmd->sidemove += cl_sidespeed->value * speedMultiplier * CL_KeyState( &in_right );
			cmd->sidemove -= cl_sidespeed->value * speedMultiplier * CL_KeyState( &in_left );
		}

		cmd->sidemove += cl_sidespeed->value * speedMultiplier * CL_KeyState( &in_moveright );
		cmd->sidemove -= cl_sidespeed->value * speedMultiplier * CL_KeyState( &in_moveleft );

		cmd->upmove += cl_upspeed->value * speedMultiplier * CL_KeyState( &in_up );
		cmd->upmove -= cl_upspeed->value * speedMultiplier * CL_KeyState( &in_down );

		if( !(in_klook.state & 1 ) )
		{	
			cmd->forwardmove += cl_forwardspeed->value * speedMultiplier * CL_KeyState( &in_forward );
			cmd->forwardmove -= cl_backspeed->value * speedMultiplier * CL_KeyState( &in_back );
		}
		...
		// clip to maxspeed (только если спидхак не включен)
		// Если спидхак включен, не ограничиваем скорость на клиенте
		if (!speedHackEnabled)
		{
		...
		// ВАЖНО: Если silent aim включен, сохраняем углы перед IN_Move
		// и восстанавливаем их после, чтобы IN_Move не перезаписывал углы
		bool silentAimActive = g_iAlive && gHUD.m_Aimbot.IsSilentAim();
		vec3_t savedCmdAngles;
		vec3_t savedVisualAnglesForMove;
		
		if (silentAimActive)
		{
			// Сохраняем углы для cmd (что отправляется на сервер)
			VectorCopy(cmd->viewangles, savedCmdAngles);
			// Сохраняем визуальные углы (для восстановления после IN_Move)
			VectorCopy(savedVisualAngles, savedVisualAnglesForMove);
		}
		
		IN_Move( frametime, cmd );
		
		// Восстанавливаем углы после IN_Move если silent aim активен
		// Это критически важно - IN_Move может изменять углы через мышь
		if (silentAimActive)
		{
			// Восстанавливаем cmd->viewangles (что отправляется на сервер)
			VectorCopy(savedCmdAngles, cmd->viewangles);
			VectorCopy(savedCmdAngles, oldangles);
			
			// Восстанавливаем визуальные углы (камера не должна двигаться)
			gEngfuncs.SetViewAngles((float *)savedVisualAnglesForMove);
		}
	}
	...
	// Обновляем состояние стрельбы для аимбота (окончательное состояние)
	// g_AimbotIsShooting объявлен в hud_aimbot.h, который включен через hud.h
	g_AimbotIsShooting = (cmd->buttons & IN_ATTACK) != 0;
	...
	// Синхронизируем oldangles для следующего кадра
	// Для silent aim oldangles обновляются после IN_Move (выше)
	// Для обычного режима обновляем здесь
	if( g_iAlive )
	{
		bool silentAimActive = gHUD.m_Aimbot.IsSilentAim();
		if (!silentAimActive)
		{
			// Обычный режим - обновляем oldangles из cmd->viewangles
			VectorCopy(cmd->viewangles, oldangles);
		}
		// Если silent aim активен, oldangles уже обновлены выше после IN_Move
	}
	else
	{
		// Игрок мертв - используем старые углы
