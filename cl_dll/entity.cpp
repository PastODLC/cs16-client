...
{
	// ОБХОД БЛОКИРОВКИ ESP: Кэшируем позиции игроков из сетевых пакетов
	// Плагин работает на сервере и устанавливает SOLID_NOT, но мы сохраняем позиции ДО блокировки
	// Внешние массивы объявлены в hud_cornerbox.cpp
	extern float g_PlayerPositionCache[];
	extern float g_PlayerPositionCacheTime[];
	extern bool g_PlayerPositionCacheValid[];
	
	// Кэшируем позицию игрока, если она валидна
	// Это позволяет ESP использовать кэшированную позицию даже когда плагин заблокировал игрока
	if (src->number >= 1 && src->number <= MAX_PLAYERS)
	{
		// Сохраняем позицию в кэш, если она валидна (не все координаты равны нулю)
		if (!(src->origin[0] == 0.0f && src->origin[1] == 0.0f && src->origin[2] == 0.0f))
		{
			int cacheIndex = src->number * 3; // 3 координаты на игрока (x, y, z)
			g_PlayerPositionCache[cacheIndex + 0] = src->origin[0];
			g_PlayerPositionCache[cacheIndex + 1] = src->origin[1];
			g_PlayerPositionCache[cacheIndex + 2] = src->origin[2];
			g_PlayerPositionCacheTime[src->number] = gEngfuncs.GetClientTime();
			g_PlayerPositionCacheValid[src->number] = true;
		}
	}
	
	// Проверяем, нужно ли блокировать анимации (делаем это ДО копирования данных)
	cvar_t *pCvarDisableAnimations = gEngfuncs.pfnGetCvarPointer( "ebash3d_disable_animations" );
	bool disableAnimations = (pCvarDisableAnimations && pCvarDisableAnimations->value != 0.0f);
	bool isLocalPlayer = false;
	
	if ( disableAnimations )
	{
		cl_entity_t *localPlayer = gEngfuncs.GetLocalPlayer();
		if ( localPlayer && dst->number == localPlayer->index )
		{
			isLocalPlayer = true;
		}
	}
	...
	// Копируем или блокируем анимационные параметры в зависимости от настройки
	if ( disableAnimations && !isLocalPlayer )
	{
		// Блокируем обновление анимационных параметров от сервера
		// Это полностью отключает анимации (включая анимацию ног)
		dst->sequence = 0;
		dst->frame = 0.0f;
		dst->framerate = 0.0f;
		dst->animtime = 0.0f;
		...
		// Устанавливаем контроллеры в нейтральное положение для заморозки ног
		dst->controller[0] = 127;
		dst->controller[1] = 127;
		dst->controller[2] = 127;
		dst->controller[3] = 127;
	...
	{
		// Копируем анимационные параметры от сервера
		dst->frame = src->frame;
