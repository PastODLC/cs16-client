...
int CHudTimer::MsgFunc_RoundTime(const char *pszName, int iSize, void *pbuf)
{
	...
	// ОЧИСТКА ESP КЭША ПРИ НОВОМ РАУНДЕ - ИСПРАВЛЯЕМ ОСТАТКИ ESP!
	// Очищаем все позиции и инвалидируем кэш при начале раунда
	extern bool g_PlayerPositionCacheValid[];
	extern float g_PlayerPositionCache[];
	extern float g_PlayerPositionCacheTime[];
	
	for (int i = 0; i <= 32; i++) // MAX_PLAYERS = 32
	{
		// ОЧИЩАЕМ ПОЗИЦИИ - убираем остатки ESP
		...
		// ИНВАЛИДИРУЕМ КЭШ - никаких остатков!
		g_PlayerPositionCacheValid[i] = false; // Невалиден по умолчанию
	}
