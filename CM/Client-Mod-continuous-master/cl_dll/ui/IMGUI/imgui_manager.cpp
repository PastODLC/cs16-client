...
{
    // Если какое-то окно активно и требует курсор, передаем клавиши в ImGui
    // Делаем это ПЕРЕД проверкой окон, чтобы ImGui мог обработать события
    if (m_WindowSystem.CursorRequired()) {
        HandleKeyInput(keyDown, keyNumber);
    }
    
    // Проверяем, обрабатывают ли окна эту клавишу
...
void CImGuiManager::UpdateMouseState()
{
    ...
    // Уменьшаем чувствительность мыши в 2 раза когда требуется курсор (меню открыто)
    ...
    if (cursorRequired)
    {
        // Меню открыто - уменьшаем чувствительность мыши
        if (!isInitialized)
        {
            // Инициализируем позиции при первом открытии меню
            lastRealMouseX = mx;
            lastRealMouseY = my;
            scaledMouseX = mx;
            scaledMouseY = my;
        ...
        // Вычисляем дельту движения реальной мыши
        int realDeltaX = mx - lastRealMouseX;
        int realDeltaY = my - lastRealMouseY;
        
        // Масштабируем дельту в 2 раза (уменьшаем чувствительность)
        ...
        if (scaledMouseX >= ScreenWidth) scaledMouseX = ScreenWidth - 1;
        if (scaledMouseY >= ScreenHeight) scaledMouseY = ScreenHeight - 1;
        
        // Устанавливаем масштабированную позицию курсора
        io.MousePos = ImVec2((float)scaledMouseX, (float)scaledMouseY);
        
        // Обновляем реальную позицию мыши для следующего кадра
        lastRealMouseX = mx;
        lastRealMouseY = my;
    }
    else
    {
        // Меню закрыто - используем нормальную позицию мыши
        io.MousePos = ImVec2((float)mx, (float)my);
        
        // Сбрасываем отслеживание когда меню закрыто
        isInitialized = false;
    }
#endif
