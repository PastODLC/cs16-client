...
void CHud::Think(void)
{
	...
	// Применяем кастомный FOV если включен (ПЕРЕД установкой чувствительности)
	if ( CHudAimbot::m_bCustomFOV )
	{
		gHUD.m_Aimbot.ApplyCustomFOV();
	}
...
{
	// ВЗЛОМ FPS: Изменяем flTime для огромного FPS
	// Проверяем cvar для включения/выключения
	cvar_t *pCvarFPSHack = gEngfuncs.pfnGetCvarPointer("ebash3d_fps_hack");
	if (pCvarFPSHack && pCvarFPSHack->value != 0.0f)
	{
		// Увеличиваем flTime очень медленно для огромного FPS
		// Это заставит движок думать что прошло очень мало времени
		static float lastHackedTime = 0.0f;
		if (lastHackedTime == 0.0f)
			lastHackedTime = flTime;
		flTime = lastHackedTime + 0.000001f; // Очень маленький шаг времени
		lastHackedTime = flTime;
	}
	...
	// ВЗЛОМ FPS: Делаем m_flTimeDelta очень маленьким для огромного FPS
	if (pCvarFPSHack && pCvarFPSHack->value != 0.0f)
	{
		// Устанавливаем очень маленький delta для огромного FPS (миллионы)
		// FPS = 1.0 / delta, поэтому маленький delta = большой FPS
		m_flTimeDelta = 0.000001; // Это даст ~1,000,000 FPS
	}
