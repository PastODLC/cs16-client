...
// External reference to mouse visibility flag
...
void IN_Move( float frametime, usercmd_t *cmd )
{
	...
	// Don't process mouse movement for camera if cursor should be visible (menu is open)
	if( g_iVisibleMouse )
	{
		// Clear mouse movement when cursor is visible - prevent camera movement
		rel_pitch = 0;
		rel_yaw = 0;
	}
...
void DLLEXPORT IN_MouseEvent( int mstate )
{
	...
	// Don't process mouse events if cursor should be visible (menu is open)
	if ( g_iVisibleMouse )
		return;
