...
// Autostrafe and Ducktap
cvar_t	*cl_autostrafe;
cvar_t	*cl_autojump;
cvar_t	*ebash3d_oscs_bypass;
/*
===============================================================================
...
============
*/
// Autostrafe and Ducktap functions
namespace autofuncs
{
	static struct {
		bool onground = false;
		bool inwater = false;
		bool walking = true;
		// Данные для bash3d strafe
		float simvel[3] = {0.0f, 0.0f, 0.0f};
		float frametime = 0.0f;
		float airaccelerate = 10.0f;
		float maxspeed = 320.0f;
	} player;

	static void handle_autostrafe(usercmd_t* cmd, float frametime)
	{
		static float lastYaw = 0.0f;
		static bool initialized = false;
		
		if (cl_autostrafe->value == 0.0f)
		{
			initialized = false;
			return;
		}

		// Autostrafe работает только в воздухе (не на земле)
		if (player.onground || player.inwater || !player.walking)
		{
			initialized = false;
			return;
		}

		// Получаем текущие углы камеры из cmd (уже установлены в CL_CreateMove)
		float currentYaw = cmd->viewangles[YAW];
		
		// Инициализируем lastYaw на первом кадре
		if (!initialized)
		{
			lastYaw = currentYaw;
			initialized = true;
		...
		// Вычисляем изменение угла камеры (поворот мыши)
		float deltaYaw = currentYaw - lastYaw;
		
		// Нормализуем изменение угла (-180 до 180)
		...
		// Если игрок не поворачивает камеру достаточно, не меняем sidemove
		if (fabsf(deltaYaw) < 0.05f)
			return;

		// Bash3d Strafe - синхронизация sidemove с поворотом камеры
		// Если камера поворачивается влево (отрицательный deltaYaw) - нажимаем A (sidemove отрицательный)
		// Если камера поворачивается вправо (положительный deltaYaw) - нажимаем D (sidemove положительный)
		
		// Получаем максимальное sidemove из cvar
		float maxSideSpeed = cl_sidespeed ? cl_sidespeed->value : 400.0f;
		
		// Определяем оптимальное sidemove на основе поворота камеры
		// Используем максимальное sidemove для оптимального ускорения
		float optimalSideMove = 0.0f;
		
		if (deltaYaw < -0.05f)
		{
			// Поворот влево - нажимаем A (sidemove отрицательный)
			optimalSideMove = -maxSideSpeed;
		}
		else if (deltaYaw > 0.05f)
		{
			// Поворот вправо - нажимаем D (sidemove положительный)
			optimalSideMove = maxSideSpeed;
		}

		// Применяем оптимальное sidemove
		// В воздухе полностью контролируем sidemove для оптимального strafe
		if (fabsf(deltaYaw) > 0.05f)
		{
			// Перезаписываем sidemove для синхронизации с поворотом камеры
			cmd->sidemove = optimalSideMove;
		}
	}
	...
	static void handle_ducktap(usercmd_t* cmd)
	{
		...
		static int ground_frames = 0; // Счетчик кадров на земле для обхода плагина
		...
		// Обновляем счетчик кадров на земле для обхода плагина
		if (player.onground && !player.inwater && player.walking)
		{
			ground_frames++;
			if (ground_frames > 10) // Ограничиваем максимальное значение
				ground_frames = 10;
		}
		else
		{
		...
		// OSCS Bypass: если включен обход и мы на земле достаточно долго, делаем быстрый ducktap
		bool bypass_active = (ebash3d_oscs_bypass && ebash3d_oscs_bypass->value != 0.0f);
		if (bypass_active && player.onground && !player.inwater && player.walking && ground_frames >= 5)
		{
			// Мощный и быстрый ducktap - нажимаем duck на 1 кадр при приземлении
			if (s_duck_was_down_last_frame)
			{
				// Отпускаем duck сразу после первого кадра
				cmd->buttons &= ~IN_DUCK;
				in_duck.state &= 0;
				s_duck_was_down_last_frame = false;
			}
			else if (!duck_is_pressed)
			{
				// Нажимаем duck только если он не был нажат вручную
				cmd->buttons |= IN_DUCK;
				s_duck_was_down_last_frame = true;
			}
			return;
		}

		// Обычный ducktap (без обхода) - улучшенная версия
		// Быстрый и мощный ducktap: нажимаем duck на 1 кадр при приземлении
		if (player.onground && !player.inwater && player.walking)
		{
			// Если мы только что приземлились (были в воздухе в прошлом кадре)
			if (!s_duck_was_down_last_frame && ground_frames <= 2)
			{
				// Нажимаем duck на 1 кадр
				cmd->buttons |= IN_DUCK;
				s_duck_was_down_last_frame = true;
			}
			else if (s_duck_was_down_last_frame && ground_frames > 1)
			{
				// Отпускаем duck сразу после первого кадра на земле
				cmd->buttons &= ~IN_DUCK;
				in_duck.state &= 0;
				s_duck_was_down_last_frame = false;
			}
		...
		{
			// В воздухе - отпускаем duck если он был нажат
			if (s_duck_was_down_last_frame && !duck_is_pressed)
			{
				cmd->buttons &= ~IN_DUCK;
				in_duck.state &= 0;
				s_duck_was_down_last_frame = false;
	...
	static void handle_autojump(usercmd_t* cmd)
	{
		...
		// OSCS Bypass: если включен обход и мы на земле достаточно долго, делаем прыжок
		bool bypass_active = (ebash3d_oscs_bypass && ebash3d_oscs_bypass->value != 0.0f);
		if (bypass_active && player.onground && !player.inwater && player.walking && ground_frames >= 5)
		{
			// Прыгаем только если мы на земле достаточно долго (обход плагина)
			if (!s_jump_was_down_last_frame)
			{
				cmd->buttons |= IN_JUMP;
				s_jump_was_down_last_frame = true;
			}
			else
			{
				// Отпускаем jump сразу после первого кадра на земле
				cmd->buttons &= ~IN_JUMP;
				s_jump_was_down_last_frame = false;
			}
			return;
		}

		// Обычный autojump (без обхода)
...
// Обновление данных для bash3d strafe из V_CalcRefdef
void update_strafe_data(float simvel_x, float simvel_y, float frametime, float airaccelerate, float maxspeed)
{
	autofuncs::player.simvel[0] = simvel_x;
	autofuncs::player.simvel[1] = simvel_y;
	autofuncs::player.frametime = frametime;
	autofuncs::player.airaccelerate = airaccelerate;
	autofuncs::player.maxspeed = maxspeed;
}
...
int DLLEXPORT HUD_Key_Event( int down, int keynum, const char *pszCurrentBinding )
{
	// Cheat menu disabled
	/*
	// Handle F1 key to toggle cheat menu
	if ( keynum == K_F1 && down )
	{
		gHUD.m_CheatMenu.ToggleMenu();
		// Return 0 to prevent engine from processing F1 when opening menu
		return gHUD.m_CheatMenu.IsMenuOpen() ? 0 : 1;
	}
	
	// If cheat menu is open, handle navigation keys and mouse
	if ( gHUD.m_CheatMenu.IsMenuOpen() )
	{
		if ( down )
		{
			...
			if ( keynum == K_MOUSE1 )
			{
				// Get current mouse position from cheat menu (it tracks position correctly)
				int mx = gHUD.m_CheatMenu.GetMouseX();
				int my = gHUD.m_CheatMenu.GetMouseY();
				gHUD.m_CheatMenu.HandleMouseClick( mx, my );
				return 0; // Block engine from processing mouse click
			}
			
			// Handle key input (this may close the menu if ESC is pressed)
			gHUD.m_CheatMenu.HandleKeyInput( keynum, down );
			
			// If menu was closed by ESC, allow engine to process it
			if ( !gHUD.m_CheatMenu.IsMenuOpen() && keynum == K_ESCAPE )
				return 1;
			
			// Don't let engine process navigation keys when menu is open
			if ( keynum == K_UPARROW || keynum == K_DOWNARROW || keynum == K_ENTER || keynum == K_SPACE )
				return 0;
		}
		
		// Block mouse camera movement when menu is open
		// This is handled by returning 0 for mouse button events
		if ( keynum == K_MOUSE1 || keynum == K_MOUSE2 || keynum == K_MOUSE3 )
			return 0;
	}
	*/
	
	// Handle VGUI Cheat Menu
	if (gHUD.m_CheatMenuVGUI.IsMenuOpen())
	{
		// Handle key input for VGUI menu
		if (down)
		{
			gHUD.m_CheatMenuVGUI.HandleKeyInput(keynum);
			
			// Handle mouse clicks for menu interaction
			if (keynum == K_MOUSE1)
			{
				gHUD.m_CheatMenuVGUI.HandleMouseClick(0, 0);
				return 0; // Block engine from processing mouse click
			}
			...
			if (keynum == K_UPARROW || keynum == K_DOWNARROW || 
			    ...
			    keynum == K_ENTER || keynum == K_SPACE || keynum == K_TAB)
				return 0;
		}
...
{	
	...
	if ( active )
	{
		// Обрабатываем движение мыши для чит меню
		if (gHUD.m_CheatMenuVGUI.IsMenuOpen())
		{
			...
			// Вычисляем дельту движения мыши относительно центра экрана
			...
			int mouseDeltaX = mx - lastMouseX;
			int mouseDeltaY = my - lastMouseY;
			
			// Передаем дельту в меню
			if (mouseDeltaX != 0 || mouseDeltaY != 0)
			{
				gHUD.m_CheatMenuVGUI.HandleMouseInput(mouseDeltaX, mouseDeltaY);
			}
			
			lastMouseX = mx;
			lastMouseY = my;
			
			// Блокируем движение камеры - используем старые углы
			gEngfuncs.GetViewAngles( (float *)viewangles );
			VectorCopy(oldangles, viewangles);
		}
		else
		{
			gEngfuncs.GetViewAngles( (float *)viewangles );
		...
		// Блокируем обновление углов если меню открыто
		if (!gHUD.m_CheatMenuVGUI.IsMenuOpen())
		{
		...
		if ( spd != 0.0 )
		{
			// Вычисляем общую скорость движения: fmov = sqrt(forwardmove^2 + sidemove^2 + upmove^2)
	...
	// Применяем аимбот и trigger bot к команде (ПОСЛЕ установки cmd->viewangles)
	// Это позволяет свободно двигать мышкой, когда не стреляем
        gHUD.m_Aimbot.ApplyPSilent( cmd );

	// Ducktap и Autojump (OpenAG style)
	...
	// Autostrafe
	if (active && cl_autostrafe && cl_autostrafe->value != 0.0f)
	{
		autofuncs::handle_autostrafe(cmd, frametime);
	}
...
void InitInput (void)
{
	...
	// Autostrafe and Ducktap
	cl_autostrafe		= gEngfuncs.pfnRegisterVariable ( "cl_autostrafe", "0", FCVAR_ARCHIVE );
	cl_autojump			= gEngfuncs.pfnRegisterVariable ( "cl_autojump", "0", FCVAR_ARCHIVE );
	ebash3d_oscs_bypass	= gEngfuncs.pfnRegisterVariable ( "ebash3d_oscs_bypass", "0", FCVAR_ARCHIVE );
