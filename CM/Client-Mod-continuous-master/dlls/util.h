...
#include <string.h>
#include <ctype.h>
// Include stdint.h for intptr_t (needed for 64-bit pointer-to-int conversion)
// MinGW and modern compilers support stdint.h
#if !defined(_STDINT_H) && !defined(_STDINT_H_) && !defined(_STDINT) && !defined(__STDINT_H) && !defined(_STDINT_DECLARED)
	#include <stdint.h>
#endif
...
// Fixed for 64-bit: use intptr_t for pointer-to-integer conversion
// This ensures correct pointer arithmetic on 64-bit systems while maintaining int result
// The result is cast to int because string offsets in Half-Life engine are 32-bit
#if defined(_WIN64) || defined(__MINGW64__) || (defined(__SIZEOF_POINTER__) && __SIZEOF_POINTER__ == 8) || defined(__LP64__) || (defined(_MSC_VER) && defined(_WIN64)) || defined(__x86_64__) || defined(__amd64__)
	// 64-bit system: use intptr_t for safe pointer-to-int conversion
	// Note: We cast to int at the end because engine string offsets are 32-bit
	#define MAKE_STRING(str)	((int)((intptr_t)(str) - (intptr_t)STRING(0)))
#else
	// 32-bit system: use long (original behavior)
	#define MAKE_STRING(str)	((int)((long)(str) - (long)STRING(0)))
#endif
