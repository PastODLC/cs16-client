...
int CHudHealth:: MsgFunc_Damage(const char *pszName,  int iSize, void *pbuf )
{
	...
	if ( damageTaken > 0 || armor > 0 )
	{
		...
		// ДЕБАГ: Регистрируем полученный урон
		// Пытаемся определить атакующего из позиции урона
		// Находим ближайшего игрока к позиции урона
		if ( damageTaken > 0 )
		{
			int attackerIndex = 0;
			float minDistance = 999999.0f;
			
			// Ищем ближайшего игрока к позиции урона
			for ( int i = 1; i <= MAX_PLAYERS; i++ )
			{
				...
				// Вычисляем расстояние между позицией урона и позицией игрока
				float dir[3];
				dir[0] = vecFrom[0] - ent->origin[0];
				dir[1] = vecFrom[1] - ent->origin[1];
				dir[2] = vecFrom[2] - ent->origin[2];
				float dist = sqrtf( dir[0] * dir[0] + dir[1] * dir[1] + dir[2] * dir[2] );
				
				if ( dist < minDistance && dist < 500.0f ) // В пределах 500 единиц
				{
					minDistance = dist;
					attackerIndex = i;
				}
			...
			// Регистрируем урон в дебаг панели
			if ( attackerIndex > 0 )
			{
				gHUD.m_Debug.RegisterDamageReceived( attackerIndex, (float)damageTaken );
			}
			else
			{
				// Если не нашли атакующего, регистрируем с индексом 0
				gHUD.m_Debug.RegisterDamageReceived( 0, (float)damageTaken );
			}
		}
	}
	return 1;
...
int CHudHealth:: MsgFunc_ScoreAttrib(const char *pszName,  int iSize, void *pbuf )
{
	...
	// ИСПРАВЛЕНИЕ ESP: Инвалидируем кэш позиции когда игрок помечен как мертвый
	// Это самое надежное место для обнаружения смерти игрока от сервера
	extern bool g_PlayerPositionCacheValid[];
	extern float g_PlayerPositionCache[];
	if (g_PlayerExtraInfo[index].dead && index > 0 && index <= MAX_PLAYERS)
	{
		g_PlayerPositionCacheValid[index] = false;
		// Обнуляем позицию в кэше для полной очистки
		g_PlayerPositionCache[index * 3 + 0] = 0.0f;
		g_PlayerPositionCache[index * 3 + 1] = 0.0f;
		g_PlayerPositionCache[index * 3 + 2] = 0.0f;
	}
...
int CHudHealth::MsgFunc_HealthInfo( const char *pszName, int iSize, void *buf )
{
	...
	if ( idx < MAX_PLAYERS )
	{
		// ДЕБАГ: Отслеживаем изменения здоровья других игроков для определения нанесенного урона
		cl_entity_t *localPlayer = gEngfuncs.GetLocalPlayer();
		if ( localPlayer && idx > 0 && idx <= MAX_PLAYERS && idx != localPlayer->index )
		{
			// Проверяем, было ли здоровье больше текущего (игрок получил урон)
			int oldHealth = g_PlayerExtraInfo[idx].sb_health;
			if ( oldHealth > 0 && health < oldHealth && health > 0 )
			{
				// Игрок получил урон - возможно, мы его нанесли
				float damage = (float)(oldHealth - health);
				// Регистрируем как нанесенный урон (если мы стреляли недавно)
				// Это приблизительное определение, но лучше чем ничего
				if ( g_AimbotIsShooting )
				{
					gHUD.m_Debug.RegisterDamageDealt( idx, damage );
				}
			}
		}
