...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	// Применяем glow эффекты к viewmodel
	if ( view && view->model )
	{
		// Используем доступ к статическим переменным через класс
		if ( CHudAimbot::m_bWeaponGlow )
		{
			gHUD.m_Aimbot.ApplyWeaponGlow();
		}
	}
	
	// Применяем glow для рук к локальному игроку
	if ( ent && CHudAimbot::m_bHandsGlow )
	{
		gHUD.m_Aimbot.ApplyHandsGlow();
	}
...
{
	// Обновляем информацию о игроке для autostrafe и ducktap
	extern void update_player_info(int onground, int inwater, int walking);
	extern void update_strafe_data(float simvel_x, float simvel_y, float frametime, float airaccelerate, float maxspeed);
	if (pparams)
	{
		int onground = (pparams->onground != -1 && pparams->onground != 0) ? 1 : 0;
		int inwater = (pparams->waterlevel > 0) ? 1 : 0;
		// Walking = не noclip и не spectator (обычный режим движения)
		int walking = (!gEngfuncs.IsNoClipping() && !pparams->spectator) ? 1 : 0;
		update_player_info(onground, inwater, walking);
		
		// Обновляем данные для bash3d strafe
		if (pparams->movevars)
		{
			float airaccelerate = pparams->movevars->airaccelerate;
			float maxspeed = pparams->movevars->maxspeed;
			update_strafe_data(pparams->simvel[0], pparams->simvel[1], pparams->frametime, airaccelerate, maxspeed);
		}
		else
		{
			// Значения по умолчанию
			update_strafe_data(pparams->simvel[0], pparams->simvel[1], pparams->frametime, 10.0f, 320.0f);
		}
	}
